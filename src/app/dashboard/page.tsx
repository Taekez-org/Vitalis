...
type Dashboard = { inicio: string; fim: string; verificadas: number; ok: number; pendentes: number; valor_em_risco: number; glosa_nao_recuperavel: number; glosa_evitada: number; aguardando_reverificacao: number; por_codigo: Record<string, number>; por_unidade: Record<string, number>; risco_por_convenio?: Record<string, number>; convenios?: string[]; convenio?: string; frescor: Freshness; ultima_carga?: { createdAt: string } };
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
         const response = await fetch(`/api/dashboard?inicio=${inicio}&fim=${fim}&convenio=${encodeURIComponent(convenio === "todos" ? "" : convenio)}`, { cache: "no-store" });
  ...
  return <section className="space-y-7">
    ...
    <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
     ...
     <div className="grid gap-4 lg:grid-cols-3"><FinancialCard label="Valor em risco" value={money(data.valor_em_risco)} caption="Todas as guias pendentes" tone="risk" /><FinancialCard label="Glosa não recuperável" value={money(data.glosa_nao_recuperavel)} caption="Autorização vencida, sessão sem cobertura, procedimento não coberto ou realizado diferente" tone="risk" /><FinancialCard label="Valor de glosa evitada" value={money(data.glosa_evitada)} caption="Guias que voltaram para OK após correção" tone="success" /></div>
     <div className="rounded-xl border border-warning/30 bg-warning/10 p-4 text-sm leading-6 text-warning-foreground"><strong>Como ler estes números:</strong> valor em risco inclui todas as pendências. Glosa não recuperável é apenas a parcela que não deve ser liberada alterando o CSV; duplicidades, conflitos de autorização e decisões de cobrança particular exigem tratamento antes do envio, mas não são automaticamente glosa.</div>
    <div className="grid gap-6 lg:grid-cols-[1.35fr_0.65fr]">
      ...
      <div className="space-y-6"><Chart title="Pendências por motivo" values={data.por_codigo} labeler={rotuloMotivo} /><Chart title="Pendências por unidade" values={data.por_unidade} /><MoneyChart title="Valor em risco por convênio" values={data.risco_por_convenio ?? {}} /></div>
...
function MoneyChart({ title, values }: { title: string; values: Record<string, number> }) { const max = Math.max(...Object.values(values), 1); return <Card className="shadow-card"><CardHeader><CardTitle>{title}</CardTitle><CardDescription>Valor deduplicado das guias pendentes</CardDescription></CardHeader><CardContent className="space-y-4">{Object.entries(values).sort(([, a], [, b]) => b - a).map(([label, value]) => <div className="grid grid-cols-[minmax(0,1.6fr)_minmax(6rem,2fr)_6rem] items-center gap-3 text-sm" key={label}><span className="break-words leading-5">{label}</span><div className="h-2.5 overflow-hidden rounded-full bg-surface-subtle"><i className="block h-full rounded-full bg-risk" style={{ width: `${(value / max) * 100}%` }} /></div><b className="text-right">{money(value)}</b></div>)}{!Object.keys(values).length && <p className="text-sm text-muted-foreground">Nenhuma glosa provável no período.</p>}</CardContent></Card>; }
